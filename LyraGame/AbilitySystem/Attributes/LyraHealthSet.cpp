...
UE_DEFINE_GAMEPLAY_TAG(TAG_Gameplay_DamageStemina, "Gameplay.Stemina");
...
void ULyraHealthSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	*/
	...
	/*
		Apply Stemina Change


	if (Data.EvaluatedData.Attribute == GetDamageSteminaAttribute())
	{
		// Send a standardized verb message that other systems can observe
		// Convert into -Stemina and then clamp
		SetStemina(FMath::Clamp(GetStemina() - GetDamageStemina(), MinimumStemina, GetMaxStemina()));
		SetDamageStemina(0.0f);
	}
	else if (Data.EvaluatedData.Attribute == GetHealingSteminaAttribute())
	{
		// Convert into +Health and then clamo
		SetStemina(FMath::Clamp(GetStemina() + GetHealingStemina(), MinimumStemina, GetMaxStemina()));
		SetHealingStemina(0.0f);
	}
	else if (Data.EvaluatedData.Attribute == GetSteminaAttribute())
	{
		// Clamp and fall into out of health handling below
		SetHealth(FMath::Clamp(GetStemina(), MinimumStemina, GetMaxStemina()));
	}	*/
...
void ULyraHealthSet::ClampAttribute(const FGameplayAttribute& Attribute, float& NewValue) const
{
	...
	{
		// Do not allow stemina to go negative or above max stemina.
		NewValue = FMath::Clamp(NewValue, 0.0f, GetMaxStemina());
	}
	else if (Attribute == GetMaxSteminaAttribute())
	{
		// Do not allow max stemina to drop below 1.
		NewValue = FMath::Max(NewValue, 1.0f);
	}
	*/
}
