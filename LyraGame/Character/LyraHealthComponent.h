...
public:
	...
	/*
	// Returns the current health value.
	UFUNCTION(BlueprintCallable, Category = "Lyra|Stemina")
	...
	// Returns the current health in the range [0.0, 1.0].
	UFUNCTION(BlueprintCallable, Category = "Lyra|Stemina")
	float GetSteminaNormalized() const;
	*/
...
public:
	...
	// Delegate fired when the max health value has changed.
	UPROPERTY(BlueprintAssignable)
	FLyraHealth_AttributeChanged OnSteminaChanged;

	// Delegate fired when the max health value has changed.
	UPROPERTY(BlueprintAssignable)
	FLyraHealth_AttributeChanged OnMaxSteminaChanged;
	*/
	...
	UPROPERTY(BlueprintAssignable)
	FLyraHealth_AttributeChanged OnEnergyChanged;

	// Delegate fired when the max Energy value has changed.
	UPROPERTY(BlueprintAssignable)
	FLyraHealth_AttributeChanged OnMaxEnergyChanged;
...
protected:
	...
	/*
	//stemina
	virtual void HandleSteminaChanged(const FOnAttributeChangeData& ChangeData);
	virtual void HandleMaxSteminaChanged(const FOnAttributeChangeData& ChangeData);
	*/
