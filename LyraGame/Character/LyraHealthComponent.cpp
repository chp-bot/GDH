...
void ULyraHealthComponent::InitializeWithAbilitySystem(ULyraAbilitySystemComponent* InASC)
{
	...
	/*
	AbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(ULyraHealthSet::GetSteminaAttribute()).AddUObject(this, &ThisClass::HandleSteminaChanged);
	AbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(ULyraHealthSet::GetMaxSteminaAttribute()).AddUObject(this, &ThisClass::HandleMaxSteminaChanged);
	*/
	...
	OnEnergyChanged.Broadcast(this, GetEnergy(), GetEnergy(), nullptr);
	OnMaxEnergyChanged.Broadcast(this, GetEnergy(), GetEnergy(), nullptr);
...
/*
float ULyraHealthComponent::GetStemina() const
{
	return (HealthSet ? HealthSet->GetStemina() : 0.0f);
}

float ULyraHealthComponent::GetMaxStemina() const
{
	return (HealthSet ? HealthSet->GetMaxStemina() : 0.0f);
}
*/

float ULyraHealthComponent::GetEnergy() const
{
	return energy;
}

void ULyraHealthComponent::SetEnergy(const float NewEnergy)
{
	energy = NewEnergy;
}

/*float ULyraHealthComponent::SetStemina(const float NewStemina) const
{
	HealthSet->SetStemina(NewStemina);
	return NewStemina;
}*/
...
/*float ULyraHealthComponent::GetSteminaNormalized() const
{
	if (HealthSet)
	{
		const float Stemina = HealthSet->GetStemina();
		const float MaxStemina = HealthSet->GetMaxStemina();

		return ((MaxStemina > 0.0f) ? (Stemina / MaxStemina) : 0.0f);
	}
...
/*
void ULyraHealthComponent::HandleSteminaChanged(const FOnAttributeChangeData& ChangeData)
{
	OnSteminaChanged.Broadcast(this, ChangeData.OldValue, ChangeData.NewValue, GetInstigatorFromAttrChangeData(ChangeData));
}

void ULyraHealthComponent::HandleMaxSteminaChanged(const FOnAttributeChangeData& ChangeData)
{
	OnMaxSteminaChanged.Broadcast(this, ChangeData.OldValue, ChangeData.NewValue, GetInstigatorFromAttrChangeData(ChangeData));
}*/

void ULyraHealthComponent::HandleEnergyChanged(const FOnAttributeChangeData& ChangeData)
{
	OnEnergyChanged.Broadcast(this, ChangeData.OldValue, ChangeData.NewValue, GetInstigatorFromAttrChangeData(ChangeData));
}

void ULyraHealthComponent::HandleMaxEnergyChanged(const FOnAttributeChangeData& ChangeData)
{
